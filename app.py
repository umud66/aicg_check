...
from analyzer import analyze_document, extract_docx, extract_pdf, results_as_dicts
...
def api_analyze():
    ...
    try:
        medium = int(request.form.get("medium_threshold", "40"))
        high = int(request.form.get("high_threshold", "65"))
    except ValueError:
    ...
    try:
        if suffix == ".docx":
            if not data.startswith(b"PK"):
                return jsonify({"error": "文件扩展名是 DOCX，但内容不像有效的 Office 文档。"}), 400
            rows = extract_docx(data)
        else:
            if not data.startswith(b"%PDF"):
                return jsonify({"error": "文件扩展名是 PDF，但内容不像有效的 PDF 文件。"}), 400
            rows = extract_pdf(data)

        results, summary = analyze_document(rows, medium, high)
    ...
    return jsonify(
        {
            ...
            "notice": "本结果是可解释的文本生成式特征风险估算，不等同于任何商业平台或学校系统的官方 AIGC 检测。",
        }
    )
